...
app = FastAPI(
    ...
    description="AI microservice for embeddings and vector storage",
    version="1.0.0",
    lifespan=lifespan,
)
...
def get_embedding_model():
    ...
    if _embedding_model is None:
        print(f"[AI Service] Loading embedding model: {_model_name}")
        from sentence_transformers import SentenceTransformer
        _embedding_model = SentenceTransformer(_model_name, device="cpu")
        print(f"[AI Service] Model loaded, dimension: {_embedding_model.get_sentence_embedding_dimension()}")
...
async def add_vector(request: VectorAddRequest):
    ...
    try:
        ...
        meta = flatten_metadata(request.meta)
        meta["_data"] = json.dumps(request.meta, ensure_ascii=False, default=str)
...
async def add_vectors_bulk(request: BulkAddRequest):
    ...
    for item in request.items:
        try:
            ...
            meta = flatten_metadata(item.meta)
            meta["_data"] = json.dumps(item.meta, ensure_ascii=False, default=str)
